...
Nodes have a :attr:`Node.left` and a :attr:`Node.right` attribute which are expected to also be instances of :class:`Node`. They can be set on initialization like so::
